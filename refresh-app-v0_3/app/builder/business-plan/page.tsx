...
export default function Builder() {
  ...
  async function saveProject(){ 
    ...
    const res = await fetch('/api/projects', {
      ...
      body: JSON.stringify({ title: `Business Plan Builder - ${topic||'Untitled'}`, type: 'business_plan', content: output })
    });
    const data = await res.json();
  ...
  async function exportPDF() {
    const res = await fetch('/api/export', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: `Business Plan Builder - ${topic||'Untitled'}`, markdown: output })
    });
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `business_plan.pdf`; a.click();
  ...
  return (
    <div className="container space-y-6">
      <div className="card">
        <h2 className="text-xl font-semibold">Business Plan Builder</h2>
        <div className="grid md:grid-cols-2 gap-4 mt-4">
          <div>
            <label className="text-sm opacity-70">Topic / Niche</label>
            <input value={topic} onChange={e=>setTopic(e.target.value)} placeholder="'Booked Smart' automation service" className="w-full border rounded-xl px-3 py-2 mt-1 bg-transparent"/>
