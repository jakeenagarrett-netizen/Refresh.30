...
export default function Builder() {
  ...
  async function saveProject(){ 
    ...
    const res = await fetch('/api/projects', {
      ...
      body: JSON.stringify({ title: `Ebook Builder - ${topic||'Untitled'}`, type: 'ebook', content: output })
    });
    const data = await res.json();
  ...
  async function exportPDF() {
    const res = await fetch('/api/export', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: `Ebook Builder - ${topic||'Untitled'}`, markdown: output })
    });
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `ebook.pdf`; a.click();
  ...
  return (
    <div className="container space-y-6">
      <div className="card">
        <h2 className="text-xl font-semibold">Ebook Builder</h2>
        <div className="grid md:grid-cols-2 gap-4 mt-4">
          <div>
            <label className="text-sm opacity-70">Topic / Niche</label>
            <input value={topic} onChange={e=>setTopic(e.target.value)} placeholder="AI for Small Businesses" className="w-full border rounded-xl px-3 py-2 mt-1 bg-transparent"/>
